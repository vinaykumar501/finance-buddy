// Load transactions and people data from localStorage
...
// === Filter section: Handle dropdown change ===
document.getElementById("filterSelect").addEventListener("change", function () {
  const selected = this.value;
  filterByRange(selected); // Apply filter based on selection
});

// On initial load, show all transactions
filterByRange("all");
...
function updateDashboard(data) {
  ...
  data.forEach(txn => {
    ...
    // Update to latest transaction date
...
function editNote() {
  notesBox.focus(); // Just puts cursor inside box
}
